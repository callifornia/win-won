...
Атомарність.
Транзакція повинна мати описаний вище тип все або нічого, і, що б не сталося, користувач повинен знати, в якому стані
знаходиться його транзакція.

Узгодженість. Транзакція, що досягає свого нормального завершення і тим самим фіксує свої результати. Тобто база
данних переходить з одного непротиречивого стану в інший. Інакше кажучи, кожна успішна транзакція за визначенням фіксує
лише допустимі результати.

Ізоляція. Події, що відбуваються всередині транзакції, повинні бути приховані від інших транзакцій, що одночасно виконуються.
Якби ця умова не виконувалася, то з причин, згаданих вище, транзакцію було б неможливо повернути до свого початку.


Довговічність. Після того, як транзакція завершилася та зафіксувала свої результати в базі даних, система має гарантувати,
що ці результати переживуть будь-які подальші збої.
